...
--
-- This module can be loaded with `local utils = require "user.utils"`
--
-- @module user.utils
-- @copyright 2022
...
function M.reload(quiet)
  ...
  local core_modules = { "user.bootstrap", "user.options", "user.mappings", }
  ...
  if not quiet then -- if not quiet, then notify of result
    if success then
      ...
      M.notify("Error reloading user...", vim.log.levels.ERROR)
    end
  end
...
---@param kind string The kind of icon in user.icons to retrieve
---@param padding? integer Padding to add to the end of the icon
---@param no_fallback? boolean Whether or not to disable fallback to text icon
---@return string icon
function M.get_icon(kind, padding, no_fallback)
  local icon = require("user.icons")[kind]
...
function M.toggle_term_cmd(opts)
  local terms = user.user_terminals
