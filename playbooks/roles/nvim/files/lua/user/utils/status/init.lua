...
--
-- This module can be loaded with `local init = require "user.utils.status.init"`
--
-- @module user.utils.status.init
-- @copyright 2023
...
-- @usage local heirline_component = { init = require("user.utils.status").init.breadcrumbs { padding = { left = 1 } } }
function M.breadcrumbs(opts)
  ...
  return function(self)
    ...
    if opts.max_depth and opts.max_depth > 0 then
      ...
      if start_idx > 0 then
        table.insert(children, { provider = require("user.utils").get_icon "Ellipsis" .. opts.separator, })
      end
    end
...
---@return function # The Heirline init function
-- @usage local heirline_component = { init = require("user.utils.status").init.separated_path { padding = { left = 1 } } }
...
---@return function # The Heirline init function
-- @usage local heirline_component = { init = require("user.utils.status").init.update_events { "BufEnter", { "User", pattern = "LspProgressUpdate" } } }
