...
--
-- This module can be loaded with `local provider = require "user.utils.status.provider"`
--
-- @module user.utils.status.provider
-- @copyright 2023
-- @license GNU General Public License v3.0

local M = {}

local condition = require "user.utils.status.condition"
local env = require "user.utils.status.env"
local status_utils = require "user.utils.status.utils"
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.fill }
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.signcolumn }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.numbercolumn }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.foldcolumn }
-- @see user.utils.status.utils.stylize
function M.foldcolumn(opts)
  opts = extend_tbl({ escape = false, }, opts)
  local ffi = require "user.utils.ffi" -- get AstroNvim C extensions
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.tabnr() }
function M.tabnr()
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.spell() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.paste() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.macro_recording() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.showcmd() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.search_count() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.mode_text() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.percentage() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.ruler({ pad_ruler = { line = 3, char = 2 } }) }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.scrollbar() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.close_button() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.filetype() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.filename() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.file_encoding() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.file_format() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.unique_path() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.file_modified() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.file_read_only() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.file_icon() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.git_branch() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.git_diff({ type = "added" }) }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.diagnostics({ severity = "ERROR" }) }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.lsp_progress() }
-- @see user.utils.status.utils.stylize
function M.lsp_progress(opts)
  local spinner = utils.get_spinner("LSPLoading", 1) or { "", }
  return function()
    local _, Lsp = next(user.lsp.progress)
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.lsp_client_names({ expand_null_ls = true, truncate = 0.25 }) }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.treesitter_status() }
-- @see user.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("user.utils.status").provider.str({ str = "Hello" }) }
-- @see user.utils.status.utils.stylize
function M.str(opts)
